...
| `pinned` | bool | Required. Step 12 (Focus): toggled by the context menu's "Pin to Focus"/"Unpin from Focus" (only shown for a task that is neither completed nor deleted). Read by the Focus section — a flat, hand-picked list of every `pinned && !completed` task, sorted with the same sibling comparator (`order`) the main list uses, rendered ABOVE the main list and hidden entirely when nothing is pinned. Completing a task (directly, or via a step-6 cascade) always resets this to `false` in the same write; un-completing (step 7) never restores it. Soft-deleting a task leaves it untouched — restoring from the Trash (step 9) brings the pin back |
| `inInbox` | bool — **dead field, always `false`** | Required only because `isValidTask()` in [firestore.rules](firestore.rules):52 asserts `d.inInbox is bool`; every write sets it to `false` solely to satisfy that check. Read by nothing. The Inbox feature it once supported was removed (see PROGRESS.md's "Inbox removal" entry); relaxing the rule so this field can be dropped entirely is a deliberate, separate follow-up requiring a rules deploy, not done here. Documents created before this change keep `inInbox: true` until a future whole-document `saveTask` rewrites them — harmless, no migration script, the same treatment the dead `colors` field (below) already got |
...
| `dueDate` | timestamp \| null | Step 13 (Dates): a Firestore Timestamp at **local midnight** of the due day, or `null`. Set/cleared inline (click the row's due-date display, or the context menu's "Set/Change due date"), committed via whole-document `saveTask` on blur — never `updateDoc`. Parsed from the `<input type="date">`'s `"YYYY-MM-DD"` value with `new Date(year, month-1, day)` (never the string form of `new Date(...)`, which parses as UTC and lands a day early west of Greenwich); read back the same way in reverse. Read by the Overdue screen (`isOverdueTask`, render.js): overdue iff the due date's local calendar day is strictly before today's — due today is not overdue. Step 18 (Recurrence) is the second writer: completing a recurring task advances this to the next occurrence instead of the task ever showing as completed. Never mentioned by `isValidTask()`, so no rules change was needed |
| `recurrence` | map \| null | Step 18 (Recurrence). `null` means "does not repeat" — also the ONLY way a recurrence ever stops (deleting the task is the other, but that's not a field value; there is deliberately no separate "stop repeating" menu action, per product-spec.md §5:145-148). One of four shapes: `{kind:'daily'}`; `{kind:'weekdays', days:number[]}` (0=Sun..6=Sat, non-empty); `{kind:'weekly', anchorDay:number}` (0=Sun..6=Sat, preserves that weekday); `{kind:'monthly', anchorDayOfMonth:number}` (1..31, re-anchored from this stored value on every advance — e.g. Jan 31 → Feb 28/29 → Mar 31 — never re-derived from the current, possibly month-end-clamped, due date). Set via the context menu's "Set/Change recurrence" (a `prompt()`-based editor, `handleSetRecurrenceClick` in app.js); a recurring task with no due date has one defaulted to today at that same moment, since a rule with no anchor cannot advance. Advancing (`advanceRecurrence`, `public/recurrence.js`) steps the rule forward from the current due date REPEATEDLY until the result is strictly after today — not one step from the old due date — so a task completed several days late lands on the next *future* occurrence rather than staying overdue. Completing a recurring task never sets `completed: true` and never runs the cascade-complete walk (step 6) — the checkbox handler branches on this field before either. Not constrained by `isValidTask()` (confirmed by reading it, not assumed), so no rules change was needed |
| `occurrenceStart` | timestamp \| null | Step 18 (Recurrence). `null` on every task that has never had a recurrence rule advance it — `render.js`'s age calculation reads `occurrenceStart ?? createdAt`, so this fallback is what keeps every pre-step-18 (and every never-recurring) task's age byte-identical with no backfill. Re-stamped to the MOMENT of each advance (i.e. to "today," not to the new, possibly-far-future, due date) every time a recurring task is completed — this is what makes a recurring task's age reset to "today" on each cycle regardless of how far out its next due date lands, matching product-spec.md:141-142's "a daily task never reports itself as months old." Not constrained by `isValidTask()`, so no rules change was needed |
...
| `weekStart` | `'sunday'` \| `'monday'` | Step 20 (Search — advanced). Read by `public/searchQuery.js`'s `this week` date term (S20-8) to decide the first day of the calendar week; written by the Tag Settings screen's "Week starts on" `<select>` (`app.js`'s `updateWeekStart`), same whole-document `setDoc`/`enqueueMutation` shape every other settings write in this file uses. Absent ⇒ read as `'sunday'` (spec:189's default), so every settings document written before this step stays valid with no backfill. Already validated by `isValidSettings()` before this step existed (confirmed by reading the rule, not assumed) — no rules change shipped with step 20. Reads and writes carry it through untouched via `normalizeTagSettings`'s `{ ...source, tags }` spread, so a color/quadrant change can never erase it out of the same whole-document write, and vice versa |
